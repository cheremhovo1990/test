...
@section('content')
    ...
    @can('manage-advert')
        <div class="d-flex flex-row mb-3">
            ...
            <a href="{{route('admin.adverts.adverts.photos', $advert)}}" class="btn btn-primary">Photos</a>
        </div>
        ...
        @if ($advert->isOnModeration() || $advert->isActive())
            <a href="{{route('admin.adverts.adverts.reject')}}" class="btn btn-danger mr-1">Reject</a>
        @endif
        ...
        <div class="d-flex flex-row mb-3">
            ...
            @if ($advert->isDraft())
                <form method="POST" action="{{route('cabinet.adverts.send', $advert)}}" class="mr-1">
                    @csrf
                    <buttton class="btn btn-success">Publish</buttton>
                </form>
            @endif
            @if ($advert->isActive())
    ...
    <div class="row">
        <div class="col-md-9">
            <p class="float-right" style="font-size">{{$advert->price}}</p>
            ...
            <div style="margin: 20px 0; border: 1px solid #ddd">
                ...
                <script>
                    ...
                    function init() {
                        var geocoder = new yamps.geocode(
                            '{{$advert->address}}',
                            {results: 1}
                        );
                        geocoder.then(
            ...
            <p>
                ...
                <span class="btn btn-primary phone-button" data-source="{{route('adverts.phone', $advert)}}"><span
                        class="fa fa-phone"><span class="number">Show Phone Number</span></span></span>
            </p>
